ncbi_and_novel_tree_final <- read.tree("files/FastTree_output_tree (3).nhx")
...
write.table(SRA_ncbi_and_novel$library, "outputs/2025.03.27.ncbi_and_novel_SRA.txt", sep = "\t", col.names = F, row.names = F, quote = T)
#do some stuff with entrez
SRA_entrez_nn <- read.table("files/sra_metadata_all_sra.txt", sep = "\t", header = F)
...
ggsave("outputs/2025.03.28.ncbi_and_novel_new.pdf", width = 250, height = 350, units = "cm", limitsize = F)

ncbi_and_novel_tree_final_pyuu <- read.tree("files/fasttree_ncbi_novel.nhx")

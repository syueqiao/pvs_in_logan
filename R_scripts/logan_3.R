...
bioproject <- read.table("../../../Downloads/SRR_Acc_List (2).txt")
feb_7_pv_PRJNA529794 <- filter(feb_7_pv, library %in% bioproject$V1)
...
write.table(select(feb_7_pv_L1_envcoords, query_acc, nuc_from, nuc_to), "feb_7_pv_L1_envcoords.bed", quote = F, col.names = F, row.names = F, sep = "\t")
...
write_file_func_2 <- function(lib){
  ...
  write.table(unique(library_filtered$query_acc), paste("2024.02.19.subgraph/", lib, "_list.txt", sep = ""), quote = F, col.names = F, row.names = F)
}
...
write.table(feb_7_L1_nuc_hmmer_env_less_90_nt, "feb_7_L1_nuc_hmmer_env_less_90_nt_ugh.txt", quote = F, col.names = F, row.names = F)
...
write.table(feb_7_L1_nuc_hmmer_env_fil_low_conf_hits_list, "feb_7_L1_nuc_hmmer_env_fil_low_conf_hits_list_ugh.txt", quote = F, col.names = F, row.names = F)
...
feb_7_pv_lib_type$pfam <- factor(feb_7_pv_lib_type$pfam, levels = c("Late_protein_L1", "Late_protein_L2", "E7", "E6", "Papilloma_E5", "Pap_E4", "PPV_E1_C", "PPV_E1_DBD", "PPV_E1_N", "PPV_E2_C", "PPV_E2_N"))
...
verts_0306 <- read.table("verts_info_clean_final_0306.txt", fill = T)
...
all_pr_fullness <- hmmsearch_clean("all_pr_need_hmmer.pep.domtbl")
...
write.table(all_pr_full_contigs, "all_pr_full_contigs.list", sep = "\t", col.names = F, row.names = F, quote = F)

feb_7_pv_L1_BI_mat_all$library <- sub("_.*", "\\1", feb_7_pv_L1_BI_mat_all$query_acc)

#from the pfam models, identify accessory genes that are on each contig
all_pr_addt_orf <- hmmsearch_clean("all_logan.domtbl")

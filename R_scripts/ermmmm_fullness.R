...
vert3 <- read.table("vert_feb7_sum.txt", sep = "\t", header = F, fill = T, col.names = c(1:25))
...
vert3_nums_info_50k_list <- vert3_nums_info_50k$other
vert3_nums_info_50k_list <- gsub("gfas/", "", vert3_nums_info_50k_list, )
vert3_nums_info_50k_list <- gsub("_sub_50_18k_test.gfa", "", vert3_nums_info_50k_list, )

vert3_nums_info_50k_list <- unique(vert3_nums_info_50k_list)
write.table(vert3_nums_info_50k_list, "feb7_50k_under_unbug.txt", sep = "\t", col.names = F, row.names = F, quote = F)
...
####run 2
# look at fullness
hmmsearch_full <- function(input_domtbl){
...
pr_run2 <- hmmsearch_full("feb7_all_pr_pilot_outputs.domtbl")
...
##clustered
pr_run3 <- hmmsearch_clean("feb7_all_pr_pilot_outputs.domtbl")
...
pr_run3_tab_fulls$library <- gsub("Score.*","",pr_run3_tab_fulls$query_acc)
...
write.table(pr_run3_tab_fulls$query_acc, "feb7_all_pr_pilot_outputs_fulls.txt", sep = "\t", col.names = F, row.names = F, quote = F)
...
pr_run5$library <- gsub("Score.*","",pr_run5$query_acc)
pr_run5 <- pr_run5 %>% separate(library, into = c("library", "library2"), sep="(?<=SRR)")
pr_run5$library2 <- gsub("SRR.*","",pr_run5$library2)
pr_run5$library_final <- paste(pr_run5$library, pr_run5$library2)
pr_run5$library <- pr_run5$library_final
...
pr_run5_tab_fours <- filter(pr_run5_tab, sum == 4)


pr_run5_tab_fulls$library <- gsub("Score.*","",pr_run5_tab_fulls$query_acc)
...
write.table(pr_run5_tab_fulls$query_acc, "mar25_all_pr_pilot_outputs_fulls_remaining.txt", sep = "\t", col.names = F, row.names = F, quote = F)
...
##clustered
pr_run5 <- hmmsearch_clean("feb7_all_pr_pilot_outputs_remaining_sort_centroids_nuc_pep.domtbl")
pr_run5_b <- filter(pr_run5, pfam == "ncbi_and_pave_L1_B")
pr_run5$library <- gsub("Score.*","",pr_run5$query_acc)
...
write.table(pr_run5_tab_fulls$query_acc, "feb7_all_pr_pilot_outputs_fulls_remaining.txt", sep = "\t", col.names = F, row.names = F, quote = F)

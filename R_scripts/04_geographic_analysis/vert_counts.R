...
pv_counts <- read.table("files/log_mb_counts.csv", header = TRUE, sep = ",")
...
uniq_counts <- read.table("files/uniq.counts", sep = "\t")
vert_mbs <- read.table("files/vert_counts_fr.txt", sep = "\t")

# Convert both to 2 names
uniq_counts <- separate(
  data = uniq_counts, col = V2,
  into = c("first", "second", "third"), sep = " "
)
...
p +
  geom_beeswarm(aes(y='',x=v2_v1),
  color = "darkgrey", 
  alpha = 0.9, method='compactswarm', size = 3) +
    scale_x_continuous(trans=scales::pseudo_log_trans(base = 10), 
    breaks=c(1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000)) +
    geom_point(data=vert_mbs_pvs_fil %>%filter( name_2 %in% target), 
    color="#3c5412", size = 3) +
    theme_bw()
...
ggplot(vert_mbs_pvs_fil, aes(x =log10(v2_v1) , y="")) +
  ...
  geom_jitter(alpha = 0.7, position = position_jitter(seed = 1, width = 1, height = 0.05), 
    aes(color = ifelse(v2_v1 >= 13537.00|v2_v1 <= 24787625.00, "black", NA))) +
  geom_jitter(alpha = 0.7, position = position_jitter(seed = 1, width = 1, height = 0.05),
    aes(color = ifelse(v2_v1 < 4.034271e-08|v2_v1 > 24787625.00, "#699320", NA))) +
  geom_text_repel(aes(label = ifelse(v2_v1 < 13537.00|v2_v1 > 24787625.00, name_2, "")),
    position = position_jitter(seed = 1, width = 1, height = 0.05)) +
